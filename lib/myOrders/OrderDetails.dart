                ...
                children: [
                  ...
                  Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(
                            "Name: ${address.isNotEmpty ? address['name'] : 'No name'}"),
                        Text(
                            "Mobile number: ${userData.isNotEmpty ? userData[0]['Mobile'] : 'No data'}"),
                        Text(
                            "Mail ID: ${userData.isNotEmpty ? userData[0]['mail'] : 'No data'}"),
                        Text(
                            "City: ${address.isNotEmpty ? address['city'] : 'No city'}"),
                        Text(
                            "PinCode: ${address.isNotEmpty ? address['pin'] : 'No pinCode'}"),
                        Text(
                            "Post Office: ${address.isNotEmpty ? address['post'] : 'No post'}"),
                        Text(
                            "House Name: ${address.isNotEmpty ? address['houseName'] : 'No houseName'}"),
                        Text(
                            "Near Landmark: ${address.isNotEmpty ? address['landmark'] : 'No landmark'}"),
                      ],
                    ),
                  ),
                  Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        if ((order['IsOrder_Cancel_User'] == 0) &&
                          ...
                          ElevatedButton(
                            onPressed: () async {
                              await task8_db.instance
                                  .updateOrderApproval(order['order_id'], true);
                              // await task8_db.instance.updateNotifySeller(order['order_id'],false);
                              await task8_db.instance.updateOrderCancelSeller(
                                  order['order_id'], false);
                              await task8_db.instance
                                  .updateCompleted(order['order_id'], false);
                              _refreshOrders();
                            },
                          ...
                          ElevatedButton(
                            onPressed: () async {
                              await task8_db.instance.updateOrderCancelSeller(
                                  order['order_id'], true);
                              await task8_db.instance.updateOrderApproval(
                                  order['order_id'], false);
                              await task8_db.instance
                                  .updateCompleted(order['order_id'], false);
                              _refreshOrders();
                          ...
                          ElevatedButton(
                            onPressed: () async {
                              await task8_db.instance
                                  .updateCompleted(order['order_id'], true);
                              await task8_db.instance.updateOrderCancelSeller(
                                  order['order_id'], false);
                              await task8_db.instance.updateOrderApproval(
                                  order['order_id'], false);
                          ...
                          ElevatedButton(
                            onPressed: () async {
                              await task8_db.instance
                                  .deleteSpecificOrder(order['order_id']);
                              Navigator.pop(context, true);
                            },
                            child: Text('Delete Order'),
                            style: ElevatedButton.styleFrom(
                              backgroundColor: Colors
                                  .brown, // Set the background color to red
                            ),
                          ),
                        ]
                      ],
